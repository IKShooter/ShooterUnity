...
namespace Player.Components
{
	...
	private Quaternion targetRotation;
        ...
        public MovementComponent(GameObject body, Camera cam, List<Transform> groundedRayPositions)
        {
            _body = body;
            playerCamera = cam;
            player_rotation = body.transform.rotation;
        ...
        public void Update()
        {
            ...
            _isMoving = (move_direction.magnitude > 0.1f);

            if (controller.isGrounded)
            {
                if (Physics.Raycast(_body.transform.position, vector_down, out hit_surface, 1.5f))
                {
			...
			move_speed = move_direction.magnitude * moveSpeed;
#endif
                dot = move_vector.x * move_direction.x + move_vector.y * move_direction.y +
                      move_vector.z * move_direction.z;
                ...
                if (move_vector.y > -0.5f)
                    move_vector.y =
                        -0.5f; // Make sure there is always a little gravity to keep the character on the ground.
            ...
            else
            {
                ...
                {
                    RaycastHit hit_up;
                    if (!Physics.SphereCast(playerCamera.transform.position + vector_down * 0.25f, 0.3f,
                            new Vector3(0, 1.0f, 0), out hit_up, 0.3f)) // Check if there is a space for player to raise
                    {
 ...
 Quaternion.Slerp(playerCamera.transform.rotation, targetRotation, frame_time * (1.0f - mouseSmoothing) * 50.0f);
#else
